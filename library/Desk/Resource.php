...
class Desk_Resource
{
  ...
  public function destroy()
  {
    ...
    $self     = $this->_definition['_links']['self']['href'];
    $response = $this->_client->delete($self);
  ...
  public function __get($name)
  {
    ...
    if (isset($this->_definition[$name])) {
      return isset($this->_changes[$name]) ? $this->_changes[$name] : $this->_definition[$name];
    } elseif (isset($this->_definition['_links'][$name])) {
      return $this->_definition['_links'][$name];
    } elseif (isset($this->_definition['_embedded'][$name])) {
  ...
  public function __isset($name)
  {
    ...
    return isset($this->_definition[$name])
      || isset($this->_definition['_links'][$name])
      || isset($this->_definition['_embedded'][$name]);
  }
  ...
  private function _cleanBaseUrl()
  {
    ...
    return preg_replace($pattern, '', explode('?', $this->_definition['_links']['self']['href'])[0]);
  }
  ...
  {
    $response = $this->_client->get($this->_definition['_links']['self']['href']);
    $this->_definition = Zend_Json::decode($response->getBody());
    $this->_loaded = true;
    $this->_setup();
  }
